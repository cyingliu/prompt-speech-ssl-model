...
class TransformerLayer(nn.Module):
    def __init__(self, config, output_attentions=False, keep_multihead_output=False, prompt_type="None", prompt_len=10, prompt_init=False, layer=0):
        ...
        if self.prompt_type == "prefix":
            ...
            emb_dim = 768
            if self.prompt_init:
                emb = torch.load(f'tera_emb_weight/layer{layer}.pt') # (768)
                print(f"Load embedding weight from tera_emb_weight/layer{layer}.pt")
                emb = emb.view(1, 1, emb_dim).expand(-1, self.prompt_len, -1)
                self.prompt = nn.Parameter(emb, requires_grad=True)
            else:
                self.prompt = nn.Parameter(torch.ones((1, self.prompt_len, emb_dim), requires_grad=True))

    def forward(self, hidden_states, attention_mask, head_mask=None, layer=0, first_chunk=True):
        # hidden_states: (1, 101, 768)
        # attention_mask: (1, 1, 1, 101) -0.
        # head_mask: None
        ...
        # torch.save(init_emb, os.path.join('/home/u7930486/chihying/s3prl/s3prl/tera_emb_weight', f"layer{layer}.pt"))
        
        ### prompt
        if self.prompt_type == "prefix" and first_chunk:
            bs = hidden_states.shape[0]
            temp_prompt = self.prompt.expand(bs, -1, -1)
            hidden_states = torch.cat((temp_prompt, hidden_states[:, self.prompt_len:, :]), dim=1)
...
class TransformerEncoder(nn.Module):
    def __init__(self, config, output_attentions=False, keep_multihead_output=False):
        ...
        layer = TransformerLayer(config, output_attentions=output_attentions,
                                  keep_multihead_output=keep_multihead_output, 
                                  prompt_type=self.prompt_type, prompt_len=self.prompt_len, prompt_init=self.prompt_init, layer=0)
        def build_layer(n_layer):
            return TransformerLayer(config, output_attentions=output_attentions,
                                  keep_multihead_output=keep_multihead_output, 
                                  prompt_type=self.prompt_type, prompt_len=self.prompt_len, prompt_init=self.prompt_init, layer=n_layer)
        if config.share_layer:
            self.layer = nn.ModuleList([layer for _ in range(config.num_hidden_layers)])
        else: # default
            self.layer = nn.ModuleList([build_layer(i) for i in range(config.num_hidden_layers)])
        if self.pre_layer_norm:
        ...
        if sys_args['--prompt'] == "prefix":
            print("Prefix tuning!")
            emb_dim = 768
            if self.prompt_init:
                emb = torch.load('tera_emb_weight/input.pt') # (768)
                print(f"Load emb weight from tera_emb_weight/input.pt")
                emb = emb.view(1, 1, emb_dim).expand(-1, self.prompt_len, -1)
    ...
    def forward(self, hidden_states, attention_mask, output_all_encoded_layers=True, head_mask=None, first_chunk=True):
        # hidden_states (1, 101, 768)
        # attention_mask -0. (bs, 1, 1, 101)
        # head_mask None, None, None
        
        # get the initialiazation of wav embedding
        # print("Save input wav embedding!!")
        # init_emb = torch.mean(hidden_states, dim=(0, 1)).detach().cpu()
        # torch.save(init_emb, os.path.join('/home/u7930486/chihying/s3prl/s3prl/tera_emb_weight', f"input.pt"))
        
        if self.prompt_type == "prefix" and first_chunk:
            bs = hidden_states.shape[0]
            temp_prompt = self.prompt_input.expand(bs, -1, -1)
            hidden_states = torch.cat((temp_prompt, hidden_states), dim=1)
        
            prompt_attention_mask = -torch.zeros((bs, 1, 1, self.prompt_len))
        ...
        for i, layer_module in enumerate(self.layer):
            ...
            hidden_states = layer_module(hidden_states, attention_mask, head_mask[i], layer=i, first_chunk=first_chunk)
            if self.output_attentions:
...
class TransformerModel(TransformerInitModel):
    ...
    def forward(self, spec_input, pos_enc=None, attention_mask=None, output_all_encoded_layers=True, head_mask=None, first_chunk=True):
