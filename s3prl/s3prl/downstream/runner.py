...
class Runner():
    ...
    def __init__(self, args, config):
        ...
        # load pretrained downstream weight
        if args.downstream_pretrain_path != 'None' and self.args.mode == "train":
            print(f"Loading pretrained downstream weight from {args.downstream_pretrain_path}")
            ckpt =  torch.load(self.args.downstream_pretrain_path, map_location='cpu')
            init_weight = ckpt.get("Downstream")
            self.downstream.model.load_state_dict(init_weight)
    ...
    def _init_model(self, model, name, trainable, interfaces=None):
        ...
        if is_initialized() and (trainable and any((p.requires_grad for p in model.parameters())) or name == "Upstream" and (self.args.prompt[0] == "prefix" or self.args.prompt[0] == "preinput")):
    ...
    def train(self):
        ...
        trainable_models = []
        trainable_paras = []
        prompt_lst = [] # add prompt paras to optimizer
        ...
        for entry in self.all_entries:
            ...
            if entry.trainable:
                ...
                print(f"Trainable {entry.name}!")
            else:
                entry.model.eval()
                print(f"NOT Trainable {entry.name}")
            ...
            if (self.args.prompt[0] == "prefix" or self.args.prompt[0] == "preinput") and entry.name == "Upstream":
                ...
                for  name, param in entry.model.named_parameters():
                    if "prompt" in name:
                        prompt_lst.append(param)
                        param.requires_grad = True
                        print("Prompt!!",name)
                trainable_paras += list(prompt_lst)
                
        # calculate trainable parameters
        # print("=" * 50, "\nTrainable Parameters\n", "=" * 50)
        # for entry in self.all_entries:
        #     model_parameters = filter(lambda p: p.requires_grad, entry.model.parameters())
        #     params = sum([np.prod(p.size()) for p in model_parameters])
        #     print(f"{entry.name}: {params}")
        #     if entry.name == "Upstream":
        #         prompt_paras = []
        #         for name, paras in entry.model.named_parameters():
        #             if "prompt" in name:
        #                 prompt_paras.append(paras)
        #         paras = sum([np.prod(p.size()) for p in prompt_paras])
        #         print(f"prompt: {paras}")
        
        # exit()
        
        # optimizer
        optimizer = self._get_optimizer(trainable_models, prompt_lst)
        ...
        while pbar.n < pbar.total:
            ...
            for batch_id, (wavs, *others) in enumerate(tqdm(dataloader, dynamic_ncols=True, desc='train', file=tqdm_file)):
                ...
                try:
                    ...
                    if self.args.get_init:
                        print("Get init of wav embedding!")
                        features = self.upstream.model(wavs)
                        exit()
                    if self.upstream.trainable or self.args.prompt[0] == 'prefix' or self.args.prompt[0] == 'preinput':
                        features = self.upstream.model(wavs)
                    else:
                        with torch.no_grad():
                            features = self.upstream.model(wavs)
    ...
    def evaluate(self, split=None, logger=None, global_step=0):
        ...
        # calculate average wav length
        # wav_lens = []
        # for batch_id, (wavs, *others) in enumerate(dataloader):
        #     wavs = [torch.FloatTensor(wav).to(self.args.device) for wav in wavs]
        #     for wav in wavs:
        #         wav_lens.append(len(wav))
        # print(f"Average wav length: {np.mean(wav_lens)}, std: {np.std(wav_lens)}")
        # print(f"Max wav length: {max(wav_lens)}")
        # exit()
        batch_ids = []
        records = defaultdict(list)
    ...
    def visualize(self):
        print('visualize!')
        # IC
        # dataset = self.downstream.model.train_dataset
        # ASR
        dataloader = self.downstream.model.get_dataloader("test-clean")
        dataset = getattr(self.downstream.model, "test-clean_dataset", None)
        for i in range(len(dataset)):
            wav, label, path = dataset[i]
            if 160 * 320 <= len(wav[0]) <= 200 * 320:
                break
        if len(wav) > 200 * 320:
            print("No enough short wav!")
            exit()
        # wav, label, path = dataset[0]
        print(f"data path: {path}")
        print(f"data len: {len(wav[0])}")
        # wavs = [wav] # IC
        wavs = wav # ASR
        wavs = [torch.FloatTensor(wav).to(self.args.device) for wav in wavs]
        for entry in self.all_entries:
