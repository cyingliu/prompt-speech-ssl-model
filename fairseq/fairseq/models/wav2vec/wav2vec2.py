...
class Wav2Vec2Model(BaseFairseqModel):
    ...
    def forward(
        ...
        features = self.dropout_input(features) ### dropout
        unmasked_features = self.dropout_features(unmasked_features) ### dropout
    ...
    def __init__(
        ...
        def block(
            ...
            if is_layer_norm:
                ...
                # return nn.Sequential(make_conv(), nn.GELU()) ### dropout
...
class TransformerEncoder(nn.Module):
    def build_encoder_layer(self, args: Wav2Vec2Config, prompt_type, prompt_len, prompt_init, downstream_task, get_init, upstream_model, layer):
    ...
    def __init__(self, args: Wav2Vec2Config):
        ...
        if '-u' not in sys_args: sys_args["-u"] = "None"
        ...
        self.upstream_model = sys_args['-u']
        ...
        if sys_args['--prompt'] == "prefix" or sys_args['--prompt'] == "preinput":
            ...
            emb_dim = 768
            if self.prompt_init:
                emb = torch.load(f'{self.upstream_model}_{self.downstream_task}_emb_weight/input.pt') # (768)
                print(f"Load emb weight from {self.upstream_model}_{self.downstream_task}_emb_weight/input.pt")
        ...
        self.layers = nn.ModuleList(
            [self.build_encoder_layer(args, self.prompt_type, self.prompt_len, self.prompt_init, self.downstream_task, self.get_init, self.upstream_model, i) for i in range(args.encoder_layers)]
        )
    ...
    def forward(self, x, padding_mask=None, layer=None):
        ...
        if self.get_init:
            ...
            if not os.path.isdir(f'{self.upstream_model}_{self.downstream_task}_emb_weight'):
                os.mkdir(f'{self.upstream_model}_{self.downstream_task}_emb_weight')
            torch.save(init_emb, os.path.join(f'{self.upstream_model}_{self.downstream_task}_emb_weight', f"input.pt"))
    ...
    def extract_features(
        ...
        for i, layer in enumerate(self.layers):
            ...
            if not self.training or (dropout_probability > self.layerdrop):
                x, (z, lr) = layer(
                    x, self_attn_padding_mask=padding_mask, need_weights=False, layer=i
                )
        ...
        # undo paddding
        if pad_length > 0:
            # x = x[:, :-pad_length] # don't undo padding for self.model.encoder
...
class TransformerSentenceEncoderLayer(nn.Module):
    ...
    def __init__(
        ...
        if self.prompt_type == "prefix":
            ...
            emb_dim = 768
            if self.prompt_init:
                emb = torch.load(f'{self.upstream_model}_{self.downstream_task}_emb_weight/layer{layer}.pt') # (768)
                print(f"Load embedding weight from {self.upstream_model}_{self.downstream_task}_emb_weight/layer{layer}.pt")
    ...
    def forward(
        ...
        # get the initialiazation of wav embedding
        if self.get_init:
            print(f"Save layer {layer} embedding!!!")
            init_emb = torch.mean(x, dim=(0, 1)).detach().cpu()
            torch.save(init_emb, os.path.join(f'{self.upstream_model}_{self.downstream_task}_emb_weight', f"layer{layer}.pt"))
        
        ### prompt
        if self.prompt_type == "prefix":
            bs = x.shape[1]
            temp_prompt = self.prompt.expand(-1, bs, -1)
            x = torch.cat((temp_prompt, x[self.prompt_len:]), dim=0)
            
        
        residual = x # (50, 1, 768) (sen len, batch size, embed dim)
        ...
        if self.layer_norm_first:
            ...
            x, attn = self.self_attn(
                ...
                value=x,
                key_padding_mask=self_attn_padding_mask,
                need_weights=True, # default False
            )
        ...
        # visualize
        # print(f"saving attn weight to attn_weight/hubert_asr_pre50_asrinit_ldim512_projdim64_short/layer{layer}.pt")
        # torch.save(attn.detach().cpu(), os.path.join('/home/u7930486/chihying/s3prl/s3prl/attn_weight', 'hubert_asr_pre50_asrinit_ldim512_projdim64_short',f"layer{layer}.pt"))
